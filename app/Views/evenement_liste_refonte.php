...
<?php include('menu.php'); $image = 'https://images.indianexpress.com/2020/06/online-classes.jpg'; ?>
 ...
 <div class="container-fluid">
        <div class="row no-gutters align-items-start"> 
            ...
            <div class="row padd" style="padding-left:3%;"> <h3><b>Evenements</b> <label id="count"> <?php echo ' ('. count($event).')'; ?></label></h3></div>
            <div class="row whitebackground" style="margin:1% !important;padding:3% !important;border-radius:1% !important"> 
                 <div class="row"> 
                    <div class="col-lg-6 col-xs-8 col-md-6 col-sm-6" style="display:flex"> 
					
                        <div class="col-lg-10" style="padding-left:0px;padding-right:0px"> 
                            <div  class="inputsearch" >
                                <input class="form-control" style="width:100%;" name="search" id="search-input" placeholder="Rechercher un évènement"/> 
                            </div>
                         </div>
                        <div class="col-lg-2" style="padding-left:0px"> 
                            <button class="btn" type="submit" id="search-button-event" style="background-color:#4ECDC4 !important;color:white" >
                                <span class="glyphicon glyphicon-search"></span>
                            </button>
                        </div>
                    </div>		
                    <div class="col-lg-2 col-xs-4 col-md-6 col-sm-6" style="text-align:right"> <button id="btnFilter" class="btn text-center"><span style="width:15px;height:15px;background: transparent url('<?php echo base_url(); ?>/assets/img/ico-filter.png') 0% 0% no-repeat padding-box;
opacity: 1;margin-top:3px;margin-right:5px;float:left;"></span> Filtres</button> </div> 
...
opacity: 1;float:left;width:15px;height:15px"></span>Nouvel &eacute;venement</button></a> </div>  
                ...
                <div id="filterContainer"  class="row" style="padding-left:3%;padding-right:0px; display:none" > 
                     <hr style="margin-right:15px !important"/> 
					...
					<div class="form-group formgroup">
						...
						<?php }?>
                    <div class="form-group" style="float:right;line-height: 40px; vertical-align:middle;margin-right:1%"> <a href="" style="color:grey !important;padding-right:10px">R&eacute;initialiser</a> </div> 
                </form></div> 
  ...
  <!--div class="form-group" style="float:right;line-height: 40px; vertical-align:middle;"> <a href="" style="color:grey !important;padding-right:10px">Annuler la s&eacute;lection</a> </div--> 
                ...
                <div style="overflow-y: scroll; height:350px; width: auto;">  
                        <table class="table" id="tableContainer"> 
                                ...
                                <?php $ind = 0; foreach ($event as $row): 
                                  ?> 
            ...
            <div class="row" style="padding-left:6%"> <h3 class=""><b>Points de ventes</b> <label id="count"><?php echo ' ('. count($pointOfSaleList).')'; ?></label></h3></div>
             <div class="row whitebackground">
                <div class="row" style="padding:5%">     
                    <button class="btn vert-button" type="button" data-toggle="modal" data-target="#modalpdv" onclick="ablechamp()" style="background-color:#4ECDC4 !important;color:white;width:100%"><span style="margin-top: 3px;
margin-left: 10px;background: transparent url('<?php echo base_url(); ?>/assets/img/ico-+.png') 0% 0% no-repeat padding-box;
opacity: 1;float:left;width:15px;height:15px"></span> Nouveau point de vente</button> </div> 
                    <hr  style="margin-top:5px !important;margin-bottom:5px !important"/>
                <div class="row" style="padding:5%;display:flex"> 
                        <div class="col-lg-10" style="padding-left:0px;padding-right:0px"> 
                            <div  class="inputsearch">
                                <input class="form-control" id="search-input-pos" name="searchpos" placeholder="Chercher un point de vente"/> 
                            </div>
                         </div>
                        <div class="col-lg-2" style="padding-left:0px"> 
                            <button class="btn" type="submit" id="search-button-pos" style="background-color:#4ECDC4 !important;color:white" >
                                <span class="glyphicon glyphicon-search" ></span>
                            </button>
                        </div>
                    </div> <br/> 
                    <div id="POSContainer" class="row "  style="margin-right:2%;padding:5%;overflow-y: scroll; height:350px; width: auto;"> 
                            <?php 
							foreach ($pointOfSaleList as $posRow){?>
                    <div id="row" style="padding-left:4%" onclick="showmodal('<?php echo $posRow->name.','.$posRow->idPointOfSale.','.$posRow->lat.','.$posRow->lng; ?>', '<?php echo $_SESSION['userType']?>')"><b>  <?php  echo $posRow->name; ?></b>   <span class="icodotes" style="float:right" width=25 height=25></span>  
                   </div> <hr/>
                    <?php } ?>
                    </div>  
                </div>  
...
<!-- SCRIPTS -->
 ...
 var arrayToValidate= new Array();
  ...
  function showmodal(info,typeUser)
  {
		...
		if(typeUser == 'organisateur'){
			$('#modalpdv').find('input[name="libpdv"]').attr('disabled',true);
			$('#modalpdv').find('input[name="latpos"]').attr('disabled',true);
			$('#modalpdv').find('input[name="lngpos"]').attr('disabled',true);
		}
 ...
 function infoEvent(id)
  ...
  function addPdv(){
   ...
   $( "#search-button-event" ).click(function() {
       ...
       type : 'POST', // Le type de la requête HTTP.
        ...
        $.each(data, function(k, v) {
			...
			if(datedebutEvent ==  dateFin){
                        dateAffiche=  '<p style="font-size:12px !important"> de '+heuredeb+' à '+heurefin+'</p>'; 
                     }
                     else{
                        dateAffiche= ' <p style="font-size:12px !important"> de '+heuredeb+' au '+datefinEvent+' '+heurefin+'</p>'; 
                     }
			str = '<tr onclick="infoEvent('+v.idEvent+')">';
            str+= '<td>'+v.name+'</td>';
            str+= '<td>'+datedebutEvent+' '+dateAffiche+'</td>';
            str+= '<td>'+v.namePlace+'<p style="font-size:12px !important">'+v.nameCity+'</p></td>';
      ...
      $.ajax({
       ...
       type : 'POST', // Le type de la requête HTTP.
		...
		if(organizer == '-1' && category == '-1' ){
			strtoapp+=   '<th scope="col"><input id="selection" onclick="seleAll('+$("#status").val()+')" type="checkbox" name="selAll" value="all"/><label for="selection"></label></th>';
		}
		else if(organizer == '-2' && category == '-1' ){
			strtoapp+=   '<th scope="col"><input id="selection" onclick="seleAll('+$("#status").val()+')" type="checkbox" name="selAll" value="all"/><label for="selection"></label></th>';
		}
        ...
        $.each(data, function(k, v) {
            ...
            str+= '<td onclick="infoEvent('+v.idEvent+')">'+v.namePlace+'<p style="font-size:12px !important">'+v.nameCity+'</p></td>';
	...
	$('#modalpdv').on('hidden.bs.modal', function () {
    $(this).find('form').trigger('reset');
	 $('#modalpdv').find('#titleModal').text("Nouveau point de vente");
	})
	
	function ablechamp()
	{
			$('#modalpdv').find('input[name="libpdv"]').attr('disabled',false);
			$('#modalpdv').find('input[name="latpos"]').attr('disabled',false);
			$('#modalpdv').find('input[name="lngpos"]').attr('disabled',false);
			$('#modalpdv').find('input[name="idpos"]').val("");
