  ...
  <section id="container">
    ...
    <section id="main-content">
      <section class="wrapper">
        <h3><i class="fa fa-angle-right"></i> Ville</h3>
      ...
      <?php if (! empty($cityList) && is_array($cityList)) : ?> 
        ...
        <?php foreach ($cityList as $row): ?>
                  <tr>
                    <td>
                     ...
                     <td>
                      <a href="<?php echo base_url('city_update?id='.$row['idCity']."'"); ?>"><button class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></button></a>
                      <button class="btn btn-danger btn-xs" onclick="return deleteCity(<?=$row['idCity'];?>);"><i class="fa fa-trash-o "></i></button>
                    </td>
                  </tr>
                 <?php endforeach; ?>
                </tbody>
        ...
        <?php else : ?> 
          <h4>Aucune ville enregistr&eacute;e</h4> 
        <?php endif ?> 
            </div>

             </br>
                <div class="form-panel">
                   <h4><i class="fa fa-angle-right"></i> <?php if (!empty($city)) echo ' Mise à jour ville'; else echo ' Nouveau'; ?></h4>
              <?php $validation =  \Config\Services::validation(); ?>
              <?= $validation->listErrors() ?>
              <form  method="POST" action="<?php if (!empty($city)) {
              echo base_url('city_updateValidate');
              $name = $city->name;$idC = $city->idCity;}
               else echo base_url('city_insert');  ?>" class="form-horizontal style-form">
