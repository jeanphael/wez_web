    ...
    <section id="main-content">
      <section class="wrapper">
        ...
        <div class="mb">
          ...
          <div class="content-panel">
            <div class="adv-table">
              ...
              <table cellpadding="0" cellspacing="0" border="0" class="table table-advance table-striped table-hover" id="hidden-table-info">
                <thead>
                  <tr>
                    <th>Nom</th>
                    <th>Date</th>
          ...
          <!--th class="hidden-phone">Tarifs</th-->
                    <?php if($nocolumnValidate !=1){ echo '<th class="hidden-phone">En attente de validation</th>'; } 

					else echo '<th></th>';?>


           <!-- 
                   th class="hidden-phone">Point de vente</th>
                     <th class="hidden-phone">Nombre de like</th>
                     <th class="hidden-phone">Inscrits</th-->
                   <?php if($notpublished != 1){?> <th></th> <?php } ?>
                    <th></th>
                  </tr>
                </thead>
        <tbody>
        <?php foreach ($events as $row):
          $idE=$row['idEvent']; ?> 
                  <tr class="gradeY">
                    <td><a href="<?php echo base_url("event_details?id=".$row['idEvent']);?>"><?= $row['name'];  ?></a></td>
                    ...
                    $heureending = explode(':',explode(' ',$row['dateEnd'])[1])[0].':'.explode(':',explode(' ',$row['dateEnd'])[1])[1];
                     if($datebeginning ==  $dateending){
                       ...
                       ?></td>
                    <td class="hidden-phone"><?= $row['placeName']; ?> - <?= $row['nameCity']; ?></td>

                    <td class="center hidden-phone"><?= $row['nameOrganizer']; ?></td>
					<td class="center hidden-phone"><?= $row['nameCategory']; ?></td><!--td class="center hidden-phone"> Ar -  Ar -  Ar</td-->
                    <?php
                    $session = \Config\Services::session();     
                     if($row['isValidated'] == 0 && $_SESSION['userType']=="admin") {
                        echo "<td class='center hidden-phone'><a href='".base_url("event_validate?id=".$row['idEvent']."&name=".$row['name']."&description=".$row['description']."")."'>Valider</a></td>";
                     }else if($row['isValidated'] == 0 && $_SESSION['userType']!="admin") {
                        echo "<td class='center hidden-phone'>oui</td>";
                     }
                    else echo "<td class='center hidden-phone'>Publié</td>";
                    ...
                    ?>
                      ...
                      <?php if($notpublished != 1){?>  <td> 
                     <a href="<?php echo base_url('event_update?id='.$row['idEvent']."'"); ?>"><button class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></button></a>
                     </td> <?php } ?>
                       <td>    <button class="btn btn-danger btn-xs" onclick="return deleteEvent(<?=$row['idEvent'];?>);"><i class="fa fa-trash-o "></i></button>
                    </td>
